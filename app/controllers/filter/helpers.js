...
function checkAsoCriterion(criterion, aso) {
  if (criterion.aso) {
    if (!aso.aso || criterion.aso.toLowerCase() !== aso.aso.toLowerCase())
      return false;
  }
  return true;
}
...
function checkLinkGeoCriteria(link, geo) {
  ...
  if (link.allowGeo === true) {
    if (link.criteria.length === 0) return true;
    else {
      for (let criterion of link.criteria)
        if (checkGeoCriterion(criterion, geo)) return true;
      return false;
    }
  }
  if (link.allowGeo === false) {
    if (link.criteria_disallow.length === 0) return true;
    else {
      for (let criterion of link.criteria_disallow) 
        if (checkGeoCriterion(criterion, geo)) return false;
      return true;
    }  
  }
}
...
function inAsoBlacklist(asoBlackList, aso) {
  for (let entry of asoBlackList) {
    if (checkAsoCriterion(entry, aso))
      return true
  }
