(function() {
  ...
  function EditLinkCtrl($scope, $state, $location, $mdDialog, $stateParams, Links, Tags, Networks, Angles, Dialog, GeolocationCodes) {
    ...
    $scope.toggleLink = function(event, initState) {
      
      if (!initState) {
        $scope.link.allowGeo = true;  
      } else {
        $scope.link.allowGeo = !initState;
      }
    }
