(function () {
  ...
  function GeoBlacklistListCtrl($scope, $filter, $location, $mdDialog, GeoBlacklist, Dialog, GeolocationCodes) {
    ...
    function deleteGeolocation(ev, id) {
      ev.stopPropagation();
      ev.preventDefault();
      Dialog.showConfirm(ev, "Are you sure to remove this geolocation from blacklist?", () => {

        GeoBlacklist.delete(id, () => refresh(), () => {
          Dialog.showAlert(ev, "Request to remove geolocation from blacklist has failed.");
        });
      ...
      GeoBlacklist.newOrUpdate($scope.geo, function(response) {
        if(response.result) {
          ...
          if(response.duplicated) {
            Dialog.showAlert(ev, "Such geolocation(s) already exist in blacklist.");
          } else {
            Dialog.showAlert(ev, "Request to update blacklisted geolocation has failed.");
          }
        }
      }, function() {
        if($scope.geo._id) {
          Dialog.showAlert(
            ...
            "Request to update blacklisted geolocation has failed.",
            false,
            "OK"
         );
        } else {
          Dialog.showAlert(
            ...
            "Request to add a geolocation to blacklist has failed.",
            false,
            "OK"
         );
        }
